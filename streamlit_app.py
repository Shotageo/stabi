...
# ===== 円弧探索（グリッド簡易版：Coarse→Quick→Refine） =====
...
if best is None:
    ...
    # dataclass でも dict でも扱えるよう、dict化も保存しておく
    selected_dict = {"xc": float(slip_best.xc), "yc": float(slip_best.yc), "R": float(slip_best.R)}
    st.session_state["selected_slip"] = selected_dict
