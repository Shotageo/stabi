# stabi_lem.py  — 2-layer support (piecewise ground + interface), lightweight calcs
...
# ---------- すべり安全率（2層：基底の層で c,phi,γ を切替） ----------
def _base_soil_vectors(interface: GroundPL, soil_upper: Soil, soil_lower: Soil, xmid, y_arc):
    yint = interface.y_at(xmid)
    mask_upper = (y_arc >= yint)
    # 基底点が上層にあれば上層パラメータ、下にあれば下層
    gamma = np.where(mask_upper, soil_upper.gamma, soil_lower.gamma)
    c     = np.where(mask_upper, soil_upper.c,     soil_lower.c)
    phi   = np.where(mask_upper, soil_upper.phi,   soil_lower.phi)
...
def fs_fellenius_poly_layered(pl: GroundPL, interface: GroundPL,
    ...
    if np.any(hmid<=0): return None
    b     = dx / cos_a
    gamma, c, phi = _base_soil_vectors(interface, soil_upper, soil_lower, xmid, y_arc)
    W     = gamma * hmid * dx
    tanp  = np.tan(phi*DEG)
...
def arcs_from_center_by_entries(
    ...
    - 単層: interface=None
    - 2層:  interface!=None かつ soil_lower!=None
    """
