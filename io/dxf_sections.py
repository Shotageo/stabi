...
def _project_point_to_polyline(x: float, y: float, ali: Alignment) -> Tuple[float, Tuple[float,float]]:
    best_d2 = float("inf"); best_s = 0.0; best_xy = (ali.points[0][0], ali.points[0][1])
    for i, ((x0,y0),(x1,y1)) in enumerate(zip(ali.points[:-1], ali.points[1:])):
        ...
        if L2 <= 1e-18:
            d2 = (x-x0)**2 + (y-y0)**2
            if d2 < best_d2:
                best_d2 = d2; best_s = ali.cumlen[i]; best_xy = (x0,y0)
            continue
    ...
    if layer_hint:
        for e in msp.query("LWPOLYLINE"):
            if str(e.dxf.layer).upper().startswith(str(layer_hint).upper()):
                ...
                if len(pts) >= 2: cand.append(pts)
    if not cand:
        for e in msp.query("LWPOLYLINE"):
            pts = _polyline_points(e)
            if len(pts) >= 2: cand.append(pts)
    if not cand: raise ValueError("中心線形となる LWPOLYLINE が見つかりません。")
    def _length(ps):
...
def load_sections(dxf_path: str, layer_filter: str = "XS") -> List[SectionLine]:
    ...
    for e in msp.query("LINE"):
        if not str(e.dxf.layer).upper().startswith(layer_filter.upper()): continue
...
def attach_stationing(xs_lines: List[SectionLine], ali: Alignment) -> List[SectionLine]:
    ...
    for s in xs_lines:
        ...
        if d0 <= d1: s.station, s.foot = s0, q0
        else:        s.station, s.foot = s1, q1
        out.append(s)
