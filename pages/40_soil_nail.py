...
if not (isinstance(sel, dict) and {"xc","yc","R"} <= sel.keys()):
    st.error("『3) 円弧探索』で最小Fs円弧を確定してください。")
    st.stop()
    ...
    with c3:
        ...
        if st.button("＋このネイルを追加", type="primary"):
            st.session_state.nails.append(
                Nail(x1=x1,y1=y1,x2=x2,y2=y2,spacing=spacing,
                     T_yield=T_yield,bond_strength=bond,embed_length_each_side=emb)
            )
...
st.subheader("結果（同一円弧・同一分割）")
cA,cB = st.columns(2)
with cA: st.metric("未補強 Fs", f"{Fs_un:.3f}" if slices_best else "—")
with cB: st.metric("補強後 Fs", f"{Fs_re:.3f}" if slices_best else "—")
